...
export default function PostForm() {
  const dispatch = useDispatch(); // 1. Bu satır mutlaka olmalı
  ...
  function onSubmit(data) {
    // Form verisini API'nin beklediği formata (string birleştirme) getiriyoruz
    const yeniNot = {
      body: `${data.g1}|${data.g2}|${data.g3}`,
    };

    dispatch(notEkleAPI(yeniNot)); // 2. API thunk'ını tetikle
    history.push('/notlar'); // 3. Yönlendirmeyi yap
  }
